...
port_acquire(int port, procid_t proc_id)
{
    ...
    if (port < 0) {
        for (int i = 0; i < NPORT; i++) {
            if (ports[i].type == PORT_TYPE_FREE && ports[i].free) {
                ports[i].free = 0;
    ...
    if (ports[port].type != PORT_TYPE_FREE || !ports[port].free) return -1;
...
port_write(int port, char *buf, int n)
{
    ...
    // YOUR CODE HERE
    if (port < 0 || port >= NPORT || buf == 0 || n < 0) return -1;
